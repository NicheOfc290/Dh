...
if (args['key'] != undefined & args['target'] != undefined & args['time'] != undefined & args['proxy'] != undefined) {
    requests["get"](protone, function(one, two, three) {
        if (args['key'] == three) {	
            ...
            console.log('You no buy HTTP-STRONG | Price 900$ | @MSIDSTRESS');
        }
    })
} else {
    console.log(gradient('orange', 'white')('node HTTP-FLOODER.js --key LICENSE-MAINHSJS --target --time --proxy | @MSIDSTRESS'));
    process.exit(-1);
}
